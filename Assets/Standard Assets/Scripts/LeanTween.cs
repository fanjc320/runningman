...
public class LeanTween : MonoBehaviour
{
						...
						else
						{
							...
							if (trans.childCount > 0)
							{
								...
								try
								{
									...
									{
										Transform transform13 = (Transform)enumerator.Current;
										if (transform13.gameObject.GetComponent<Renderer>() != null)
										{
											Material[] materials2 = transform13.gameObject.GetComponent<Renderer>().materials;
						...
						else if (tweenAction == TweenAction.COLOR)
						{
							...
							if (trans.childCount > 0)
							{
								...
								try
								{
									...
									{
										Transform transform14 = (Transform)enumerator2.Current;
										if (transform14.gameObject.GetComponent<Renderer>() != null)
										{
											Material[] materials4 = transform14.gameObject.GetComponent<Renderer>().materials;
					...
					else if (tweenAction == TweenAction.TEXT_COLOR)
					{
						...
						if (trans.childCount > 0)
						{
							...
							try
							{
								...
								{
									Transform transform15 = (Transform)enumerator3.Current;
									Text component3 = transform15.gameObject.GetComponent<Text>();
									if (component3 != null)
									{
	...
	{
		cancel(gameObject, callComplete: false);
	}
	...
	public static void drawBezierPath(Vector3 a, Vector3 b, Vector3 c, Vector3 d, float arrowSize = 0f, Transform arrowTransform = null)
	{
		...
		{
			for (float num3 = 1f; num3 <= 30f; num3 += 1f)
			{
				float d2 = num3 / 30f;
				Vector3 vector2 = ((a2 * d2 + b2) * d2 + b3) * d2 + a;
				Gizmos.DrawLine(vector, vector2);
	...
	public static void addListener(GameObject caller, int eventId, Action<LTEvent> callback)
	{
		...
		UnityEngine.Debug.LogError("You ran out of areas to add listeners, consider increasing INIT_LISTENERS_MAX, ex: LeanTween.INIT_LISTENERS_MAX = " + INIT_LISTENERS_MAX * 2);
	}
