		...
		private void GenerateTextMesh()
		{
			...
			for (int j = 0; m_char_buffer[j] != 0; j++)
			{
				...
				if (m_enableWordWrapping || m_overflowMode == TextOverflowModes.Truncate || m_overflowMode == TextOverflowModes.Ellipsis)
				{
					...
					else if (((flag4 || m_isCharacterWrappingEnabled) && m_characterCount < num - 1 && !m_fontAsset.lineBreakingInfo.leadingCharacters.ContainsKey(num5) && !m_fontAsset.lineBreakingInfo.followingCharacters.ContainsKey(m_VisibleCharacters[m_characterCount + 1])) || flag5)
					{
						SaveWordWrappingState(ref m_SavedWordWrapState, j, m_characterCount);
					}
				}
