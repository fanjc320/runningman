		...
		private void GenerateTextMesh()
		{
			...
			for (int j = 0; m_char_buffer[j] != 0; j++)
			{
				...
				if (m_enableWordWrapping || m_overflowMode == TextOverflowModes.Truncate || m_overflowMode == TextOverflowModes.Ellipsis)
				{
					...
					else if ((num6 > 11904 && num6 < 40959) || m_fontAsset.lineBreakingInfo.leadingCharacters.ContainsKey(num6) || m_fontAsset.lineBreakingInfo.followingCharacters.ContainsKey(num6))
					{
						if (m_characterCount < num - 1 && !m_fontAsset.lineBreakingInfo.leadingCharacters.ContainsKey(num6) && !m_fontAsset.lineBreakingInfo.followingCharacters.ContainsKey(m_VisibleCharacters[m_characterCount + 1]))
						{
							SaveWordWrappingState(ref m_SavedWordWrapState, j, m_characterCount);
							m_isCharacterWrappingEnabled = false;
							flag4 = false;
